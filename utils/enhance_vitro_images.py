"""
Script làm nét và cải thiện chất lượng ảnh trong data vitro
Xử lý trước khi tăng cường dữ liệu (augmentation)
"""
import os
...
def enhance_image_simple(image_path: Path, output_path: Path, method: str = 'cv2', strength: str = 'balanced'):
    """
    Xử lý ảnh đơn giản (PIL) - nhanh hơn
...
def process_vitro_data(
    ...
    """
    Xử lý tất cả ảnh trong data vitro
    
    Args:
        data_dir: Đường dẫn đến thư mục in-vitro
        output_dir: Thư mục output (None = dùng data/processing/inVitro)
    ...
    if output_dir is None:
        # Tạo đường dẫn processing: data/processing/inVitro/inVitro
        project_root = data_path.parent.parent.parent  # Từ inVitro/inVitro -> data
        output_dir = str(project_root / "processing" / "inVitro" / "inVitro")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Làm nét và cải thiện chất lượng ảnh trong data vitro'
    )
    parser.add_argument(
        '--data-dir',
        type=str,
        default='data/raw/inVitro/inVitro',
        help='Đường dẫn đến thư mục in-vitro data'
    )
    parser.add_argument(
        '--output-dir',
        type=str,
        default=None,
        help='Thư mục output (None = dùng data/processing/inVitro/inVitro)'
    )
    parser.add_argument(
        '--method',
        type=str,
    ...
    print("="*70)
    print("🖼️  ENHANCE VITRO IMAGES - Làm nét và cải thiện chất lượng")
    print("="*70)
    ...
    print("   - Nên dùng --output-dir để giữ ảnh gốc, sau đó kiểm tra trước khi overwrite")
